    ...
    select
		b.BorrowerID,
			case 
				...
				WHEN DATEDIFF(YEAR, B.[Date Of Birth], GETDATE()) BETWEEN 31 AND 40 THEN '81-90'
				ELSE 'Too Old'
			END 
		as AgeGroup
	from Borrowers as b
...
Genre_AgeGroup_Score(AgeGroup, Genre, Score) as (
	select ba.AgeGroup, b.Genre, COUNT(*) as Score
...
Genre_AgeGroup_Rank (AgeGroup, Genre, [Genre Rank]) as (
    select AgeGroup, Genre, ROW_NUMBER() OVER (PARTITION BY AgeGroup ORDER BY Score DESC) AS GenreRank
    from  Genre_AgeGroup_Score
)
